...
import { KeyRound, Send, Bot, Shield, Trash2, Loader2, AlertTriangle } from 'lucide-react';

export default function AICopilotPanel() {
  const { nodes, edges, threats, openAiKey, setOpenAiKey } = useThreatModel();
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!inputMessage.trim() || !openAiKey) return;
...
`;
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: `**Error:** Failed to communicate with OpenAI API. Please check your API key or network connection. (${error.message})` }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!openAiKey) {
    return (
      <div style={{ flexGrow: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '40px' }}>
        <div className="glass-panel" style={{ maxWidth: '500px', width: '100%', padding: '40px', borderRadius: '12px' }}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            <Bot size={48} color="#8b5cf6" style={{ marginBottom: '16px' }} />
            <h2 style={{ fontSize: '24px', marginBottom: '8px' }}>AI Security Copilot</h2>
            <p style={{ color: 'var(--text-muted)' }}>Bring Your Own Key (BYOK) Architecture</p>
          </div>
          ...
          <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid #ef4444', padding: '16px', borderRadius: '8px', marginBottom: '24px', display: 'flex', gap: '12px' }}>
            ...
            <div style={{ fontSize: '14px', color: 'var(--text-main)' }}>
              <strong>Zero Backend Privacy:</strong> Your key is stored exclusively in your browser's local memory. It is never sent to our servers. All API requests go directly from your browser to OpenAI.
            </div>
          </div>
          ...
          <form onSubmit={handleSaveKey}>
            ...
            <input 
              ...
              style={{ width: '100%', padding: '12px', borderRadius: '6px', border: '1px solid var(--panel-border)', background: 'rgba(0,0,0,0.2)', color: 'white', marginBottom: '16px' }}
            />
            <button type="submit" className="btn" style={{ width: '100%', justifyContent: 'center', background: '#8b5cf6', padding: '12px', fontSize: '16px' }}>
              <KeyRound size={18} /> Connect Copilot
            </button>
          </form>
        </div>
      </div>
      ...
      <div style={{ padding: '20px 40px', borderBottom: '1px solid var(--panel-border)', background: 'var(--panel-bg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginTop: '4px' }}>
            Powered by GPT-4o. The AI has full context of your architecture.
          </p>
        </div>
        <button className="btn btn-secondary" onClick={() => setOpenAiKey('')} title="Remove API Key">
          <Trash2 size={16} /> Disconnect Key
        </button>
      </div>
